...
main = do
  ...
  -- 1. ARGV
  -- 2. Env var
  -- 3. (one day) local config fileoptions <- execParser optionsWithInfo
  -- FIXME: some commands don't need token + project-idbestToken <-
  ...
  where
    ...
    failIf cmp msg =
      case cmp of
        Nothing -> TIO.putStrLn msg >> exitFailure
        Just t -> return t
