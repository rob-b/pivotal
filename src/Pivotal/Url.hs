...
params (StoryListParams (ListParams t s)) =
  ...
  where
    f pair = isJust (snd pair)
params (StoryDetailParams _) = []

joinPath :: ProjectId -> [T.Text] -> [T.Text]
joinPath projectId segments =
  ["services", "v5", "projects", unProjectId projectId] ++ segments
...
combine b =
  T.append schemeAndLocation . decodeUtf8 . BLC.toStrict $ toLazyByteString b
...
storyPath projectId (Just s) =
  joinPath projectId ["stories", sformat ("" % int) s]

mkMembershipsURL :: ProjectId -> String
mkMembershipsURL projectId =
  T.unpack . combine . encodePathSegments $ joinPath projectId ["memberships"]
