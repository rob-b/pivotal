...
data ListParams = ListParams
  { storyType :: Maybe B.ByteString
  , storyState :: Maybe B.ByteString
  } deriving (Show)

data DetailParams =
  DetailParams Integer
   deriving (Show)

data StoriesParams
  = StoryListParams ListParams
  | StoryDetailParams DetailParams
   deriving (Show)

newtype Handler = Handler
  { unHandler :: L.ByteString -> IO T.Text
  } deriving (Show)
...
setCheckStatus options = options & checkStatus .~ (Just $ \_ _ _ -> Nothing)
